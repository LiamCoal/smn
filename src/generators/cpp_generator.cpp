...
str CPP_Generator::generate_class_end()
{
    if (!class_info._class)
    {
        print_error(global.filename + " at line " + current_line +
                    ": attempted to use `end class` when no class is being defined");
        exit(1);
    }
    if (class_info._unimpl)
    {
...
str CPP_Generator::generate_interface_end()
{
    ...
    {
        print_error(global.filename + " at line " + current_line +
                    ": attempted to use `end interface` when no interface is being defined");
        exit(1);
    }
    if (!class_info._unimpl)
    {
...
str CPP_Generator::generate_for_start(const str &v, int f, int t)
{
    if (t < f)
    {
        print_error(global.filename + " at line " + current_line + ": [for] <to> (" + std::to_string(t) +
                    ") cannot be less than <from> (" + std::to_string(f) + ")");
        exit(1);
    }
    if (t == f)
    {
...
str CPP_Generator::generate_postwhile_end(const str &condition)
{
    ...
    if (condition == "false")
    {
        print_warning(global.filename + " at line " + current_line +
                      ": single run postwhile (consider removing postwhile loop here)");
    }
    _indent--;
