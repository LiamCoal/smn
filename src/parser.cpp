...
void parse_line(str s)
{
    ...
    *global.output << generator->generate_line_preprocessor((int) global.line, global.filename);
    ...
    {
        print_warning("at line " + current_line + ": main will be removed in a future release. use `func int, main`");
    ...
    {
        print_warning("at line " + current_line + ": newptr will be removed in a future release. use `newvar`");
        str type = args.front();
        args.pop_front();
        str name_ = args.front();
        args.pop_front();
    ...
    {
        print_warning("at line " + current_line + ": setptr will be removed in a future release. use `setvar`");
        str name_ = args.front();
        args.pop_front();
        str value = args.front();
        args.pop_front();
    ...
    else if (name == "run_tests" && global.language == Language::CPP && global.is_test)
    {
        ...
        for (int j = 0; j < test_index; ++j)
        {
            *global.output <<
                           ...
                           generator->generate_function_call("eprintln", new str[1]{
                                   "\"! " + test_names[j]->substr(0, test_names[j]->length() - 5) + "\""}, 1) <<
